...
description: |
  ...
  Follows a 5-phase workflow: Plan → Verify → Prototype → Implement → Cleanup.
...
## Feature Workflow (Phases 1-5)

The following phases repeat for each feature, refactor, or enhancement. Each phase must complete before the next begins unless the user explicitly directs otherwise.
...
Before moving to the next phase, write the architecture notes to `.ai/knowledge/architecture.md` (or update the existing file). This ensures the next session or phase can reload the design context from disk rather than relying on conversation history.
...
- **Phase:** 2 (Plan Verification) — move to 3 when user approves
- **Current unit:** —
- **Last checkpoint:** Plan written and reviewed
- **Next action:** User approval or second-agent review
```

This status block is the resume point — update it at every phase transition and after each unit of work.
...
Update the status block in the plan to reflect progress through Phase 3.
...
### Phase Transition: Read from Disk

Before starting Phase 4, **do not rely on conversation history.** Read the current state from disk:

```bash
cat .ai/feature/<name>.md
```

This re-establishes context from the canonical source. The same pattern applies at every phase boundary — read the plan, read the knowledge base, proceed from there.
...
4. **Commit** if verification passes. Use a commit message that references the plan:
...
5. **Debug and fix** if verification fails, then re-verify
6. **Update the status block** in the plan file — mark the unit as complete and note the commit SHA
7. **Confirm** with the user the unit is complete before moving to the next
...
6. If the change is significant, consider running through Phase 2 (plan verification) again

---

## Phase 5 — Overview & Cleanup

### 10. Outcomes Document

Before wrapping up, produce an outcomes section in the plan file. Append to `.ai/feature/<name>.md`:
...
2. Clean up any dead code or debug artifacts
3. If any knowledge was gained that should be preserved, update `.ai/knowledge/`:
...
Present a summary of what was done — changes, test results, any deviations from the plan.
...
- If the user requests changes, make them, re-run tests, and return to review
